...
Use of multithreading to preform the API request
"""
class WebBot:
    ...
    def threaded_geocode(self):
        """
        Threaded implementation of the search_geocode method
        Each API request is now its own thread, speeding up the I/O process
        """

        def search_geocodes(city):
            """ Performs the API request initiated by threaded_geocode """
            ...
            for data in results:
                if data['admin1'] == "California":
                    temp_dict = {'latitude': data['latitude'], 'longitude': data['longitude']}
                    self.city_dict[city] = temp_dict

        threads = []
        start = time.time()  # Start the timer before starting the threads
        for city in self.cities:
            thread = thr.Thread(target=search_geocodes, args=(city,))
            threads.append(thread)
            thread.start()

        for thread in threads:
            thread.join()
        print(f"Time to execute API Request: {time.time() - start:.2f}s")

        with open('cities.json', 'w') as fh:
            json.dump(self.city_dict, fh, indent=3)
        print("Data added successfully!")

    def threaded_weather(self, cities):
        """ Threaded implementation of search_weather """
        ...
        def search_weather(city, city_info):
            ...
            weather_dict[city] = city_dict

        threads = []
        start = time.time()  # Start the timer before starting the threads
        for city in cities:
            thread = thr.Thread(target=search_weather, args=(city, city_info))
            threads.append(thread)
            thread.start()

        for thread in threads:
            thread.join()
        print(f"Time to retrieve weather data: {time.time() - start:.2f}s")
...
class MainWindow(tk.Tk):
    ...
    def listbox_names(self):
        ...
        if not os.path.exists("cities.json"):
            ...
            self.web.threaded_geocode()  # creates cities.json if it does not exist
    ...
    def request_api(self, cities):
        ...
        weather_dict = self.web.threaded_weather(cities)  # retrieve weather data using geocodes
        return weather_dict
...
geocoding data       6.13           1.28
weather data         5.92           1.36
